        ...
        \begin{tikzpicture}[in=180,out=0]
            ...
            \uncover<6>{
                \node[align=center] at (-3, -5) {
                    \emph{On dit qu'on bootstrappe OCaml X.XX} \\
                    \emph{\textbf{\`a partir} de OCaml 4.08}
                };
            }
        \end{tikzpicture}
    \end{center}
...
\begin{frame}
    ...
    \begin{center}
        ...
        \only<5->{
            \begin{tikzpicture}[in=180,out=0]
                \node [align=center] (gcc_source) at (-3, 0.5) {
                    Code source \texttt{gcc} \\
                    en \textbf{C}
                };
                \node (gcc_post) at (3, 0) {\boxed{\texttt{gcc}}};

                \draw[-latex] (gcc_source.east) to (gcc_post.west);

                \node (bootstrap_compiler) at (-3, -0.5) {\textbf{Compilateur bootstrap}};

                \draw[-latex] (bootstrap_compiler.east) to (gcc_post.west);
