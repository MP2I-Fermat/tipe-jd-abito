...
const INTEGER_CONSTANT = 0
const FLOATING_CONSTANT = 2
const ENUMERATION_CONSTANT = 1
const CHARACTER_CONSTANT = 3
...
type AndExpression
    rem May be NULL if this expression is a passthrough.
...
type Expression
    rem May be NULL if this is not a comma expression.
...
type AbstractDeclarator
    rem May be NULL if direct_abstract_declarator is non NULL.
...
type LabeledStatement
    rem Set to "default" if this is a default case.
    label as Identifier ptr
    statement as Statement_ ptr
    rem May be NULL.
    case_expression as ConstantExpression ptr
end type

const BLOCK_STATEMENT = 0
const BLOCK_DECLARATION = 1

type BlockItem
    type as uinteger
    union
        as_statement as Statement_ ptr
        as_declaration as Declaration ptr
    end union
end type

type BlockItemList
    rem May be NULL.
    prev as BlockItemList ptr
    item as BlockItem ptr
end type

type CompoundStatement
    rem May be NULL.
    items as BlockItemList ptr
end type

type ExpressionStatement
    rem May be NULL.
    expression as Expression ptr
end type

const IF_SELECTION = 0
const SWITCH_SELECTION = 1

type SelectionStatement
    type as uinteger
    expression as Expression ptr
    primary_branch as Statement_ ptr
    rem May be NULL - corresponds to the else branch for an if statement.
    secondary_branch as Statement_ ptr
end type

const WHILE_LOOP = 0
const DO_WHILE_LOOP = 1
const FOR_LOOP = 2
...
type IterationStatement
    ...
    union
        type
            as_while_expression as Expression ptr
            as_while_body as Statement_ ptr
        end type
        type
            rem May be NULL.
            as_for_declaration as Declaration ptr
            rem May be NULL.
            as_for_init_expression as Expression ptr
            rem May be NULL.
            as_for_condition as Expression ptr
            rem May be NULL.
            as_for_action as Expression ptr
        end type
    end union
end type

const GOTO_STATEMENT = 0
const CONTINUE_STATEMENT = 1
const BREAK_EXPRESSION = 2
const RETURN_EXPRESSION = 3

type JumpStatement
    type as uinteger
    rem May be NULL. Always an identifier for goto statements.
    expression as Expression ptr
end type

const LABELED_STATEMENT = 0
const COMPOUND_STATEMENT = 1
const EXPRESSION_STATEMENT = 2
const SELECTION_STATEMENT = 3
const ITERATION_STATEMENT = 4
const JUMP_STATEMENT = 5

type Statement
    type as uinteger
    union
        as_labeled_statement as LabeledStatement ptr
        as_compound_statement as CompoundStatement ptr
        as_expression_statement as ExpressionStatement ptr
        as_selection_statement as SelectionStatement ptr
        as_iteration_statement as IterationStatement ptr
    end union
end type
...
type DeclarationList
    rem May be NULL.
    prev as DeclarationList ptr
    declaration as Declaration ptr
end type

type FunctionDefinition
    specifiers as DeclarationSpecifiers ptr
    declarator as Declarator ptr
    rem May be NULL.
    declarations as DeclarationList ptr
    body as CompoundStatement ptr
end type
...
type ExternalDeclaration
    ...
    union
        as_declaration as Declaration ptr
        as_function_definition as FunctionDefinition ptr
    end union
end type

type TranslationUnit
    rem May be NULL.
    prev as TranslationUnit ptr
    declaration as ExternalDeclaration ptr
end type